...
tags:
  ...
  - Claude Code
  - 工具
description: 这是一项已归档的 MCP 实现记录。现在在 Codex 中优先使用原生 request_user_input，而非新装该服务器。
outline: deep
aside: true
---
...
这篇文章记录一个已经归档的 MCP 实现：它曾为 Codex 补充兼容 AskUserQuestion 的交互。现在应优先使用原生 `request_user_input`，不要再为新项目安装这个服务器。
...
截至 2026-07-13，Codex 已实验性支持在非 Plan 模式使用原生 `request_user_input`。对新项目，直接启用原生能力即可；下面保留的是项目当时的动机、接口与兼容性取舍，供查阅，不是新的安装指南。
...
现在建议直接启用 Codex 原生的实验功能，无需安装 MCP 服务器：

```bash
codex features enable default_mode_request_user_input
```

这会在非 Plan 模式开放 `request_user_input` 工具。下文的 MCP 配置仅保留作历史参考；其中的审批策略和命令行行为可能随 Codex 版本变化，使用前应查阅当前官方说明。
...
它当时正好卡在我最在意的一个点上：**Agent 在动手前，是不是先来问我**。grilling 解决的是“问的内容和节奏”，AskUserQuestion 解决的是“问的形式和摩擦”，mcp-ask-user-question 则是在当时的 Codex 客户端里补上这块能力。

这三件事拼在一起，开发体感才连续：Agent 会主动追问、追问有结构、而且不论我用哪个 CLI，都能用同一种方式被问到、用同一个动作回答。

对我自己来说，它曾把 grilling 这条路子，从“只能在 Claude Code / OpenCode 用”扩到了 Codex。原生能力出现后，这个项目就完成了它的历史任务。

- **GitHub 仓库**: [Sanjeever/mcp-ask-user-question](https://github.com/Sanjeever/mcp-ask-user-question)

如果你也经常用 Codex 做开发，又想让 Agent 学会“先问、再写”，请直接启用
`default_mode_request_user_input`，并以当前官方文档为准。
