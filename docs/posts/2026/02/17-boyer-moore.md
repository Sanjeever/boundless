...
第一次对齐时，模式串末尾的 `E` 会与文本中的 `P` 比较，立刻不匹配。`P` 不在 `EXAMPLE` 中，因此坏字符规则允许模式串整体右移 7 位；不必把模式串只挪 1 位，再把已经不可能匹配的字符重比一遍。

当右侧已经匹配了一段、左侧才出错时，才轮到好后缀规则发挥作用。此时有两个事实：
...
::: warning 字符集提醒
这份 Java 与 JavaScript 实现使用 256 大小的坏字符表，并通过低 8 位索引，因此只适合 ASCII 文本。它不是“遇到 Unicode 时性能变差”，而是可能把不同字符错误地当成同一个表项。若要处理一般 Unicode，应按码点遍历，并使用 `Map` 或其他完整字符表重写预处理逻辑。
:::

## 复杂度与适用场景

- 本文这个实现的预处理为 $O(m)$；搜索在许多自然语言文本中跳步明显，但复杂度结论取决于具体变体与字符模型。
- 仅用坏字符规则时，最坏情况可能退化为 $O(nm)$；包含好后缀规则的完整 Boyer-Moore 也应说明所采用的证明与变体，不能用一句“通常很快”代替边界。
- 当模式串较长、文本较大且需要理解算法机制时，它很值得学习；业务代码通常应优先使用语言标准库的字符串查找，除非性能测量证明确有必要自行实现。

## 小结

Boyer-Moore 的“反直觉”在于它不执着于逐位前进，而是利用已经看到的信息安全地跳跃。读完后不妨用 `HERE IS A SIMPLE EXAMPLE` 和 `EXAMPLE` 跑一次代码，确认返回的首次位置；能解释每次位移为何安全，才算真正掌握。
